...
struct hooktable
{
	...
	static hook<Type, Args...>* find(const std::string& identifier) {
		...
		if (_hook == table.end()) {
			//std::cout << "can't find " << identifier << '\n';
			return nullptr;
		}
		//std::cout << "can find " << identifier << '\n';
	...
	inline friend std::vector<phookbase>* GetHookTable();
...
std::vector<phookbase>* GetHookTable() { return &hooktable::table; }
